...
import net.app315.hydra.intelligent.planting.dto.gugeng.salemanage.ProductManageOrderDTO;
...
import net.app315.hydra.intelligent.planting.server.service.gugeng.salemanage.ExportDeliveryOrderPdfService;
...
@RestController
@RequestMapping(VALID_PATH+"/sale/order")
@Api(tags = "销售订单管理")
public class ProductManageOrderController {
	...
	@Autowired
	ExportDeliveryOrderPdfService deliveryOrderPdfService;
	...
	@EnableRateLimiter
	@NeedAdvancedSearch
	@PostMapping("/exportDeliveryOrderPdf")
	@ApiOperation(value = "导出配送单")
	@ApiImplicitParam(name = "super-token", paramType = "header", defaultValue = "64b379cd47c843458378f479a115c322", value = "token信息", required = true)
	public void exportDeliveryOrderPdf(Long orderId, HttpServletResponse response) throws Exception {
//		response.addHeader("Content-Type", "application/octet-stream");
//		response.addHeader("Content-Disposition", "attachment; filename=\"" + new String("配送校验单".getBytes("UTF-8"), "ISO-8859-1") + "\".pdf;filename*=UTF-8''" + new String("配送校验单".getBytes("UTF-8"), "ISO-8859-1"));
		deliveryOrderPdfService.exportPdf(orderId, response.getOutputStream());
	}

	/**
	 * 导出excel
	 */
	@GetMapping("/exportDeliveryOrderPdf/{id}")
	@ApiOperation(value = "导出配送单")
	@ApiImplicitParam(name = "super-token", paramType = "header", defaultValue = "64b379cd47c843458378f479a115c322", value = "token信息", required = true)
	public void exportDeliveryOrderPdf2(@PathVariable("id") Long orderId, HttpServletResponse response) throws Exception {
//		response.addHeader("Content-Type", "application/octet-stream");
//		response.addHeader("Content-Disposition", "attachment; filename=\"" + new String("配送校验单".getBytes("UTF-8"), "ISO-8859-1") + "\".pdf;filename*=UTF-8''" + new String("配送校验单".getBytes("UTF-8"), "ISO-8859-1"));
		deliveryOrderPdfService.exportPdf(orderId, response.getOutputStream());
	}
