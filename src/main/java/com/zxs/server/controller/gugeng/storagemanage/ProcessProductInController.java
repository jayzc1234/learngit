...
import static net.app315.hydra.intelligent.planting.common.gugeng.constants.message.error.InStorageErrorMsgConstants.ENTER_AT_LEAST_ONE_INBOUND_DATA;
import static net.app315.hydra.intelligent.planting.common.gugeng.constants.message.error.InStorageErrorMsgConstants.PLANT_BATCH_ID_IS_NOT_NULL;
 ...
 */
@RestController
@RequestMapping(VALID_PATH+"/process/product/in")
@Api(tags = "加工产品入库管理")
public class ProcessProductInController extends CommonUtil {
    ...
    public RestResult save(@RequestBody @Validated ProcessProductInDTO processProductInDTO) throws SuperCodeException {

        AddSortStorageRequestDTO requestDTO = new AddSortStorageRequestDTO();
        BeanUtils.copyProperties(processProductInDTO,requestDTO);
        // 校验分拣入库至少录入一个入库数值数据
        ...
        requestDTO.setSortingType((byte)3);
        requestDTO.setInType((byte) 2);
