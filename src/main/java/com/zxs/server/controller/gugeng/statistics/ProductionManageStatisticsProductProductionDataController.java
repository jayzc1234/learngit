 ...
 */
@RestController
@RequestMapping(VALID_PATH+"/productionManageStatisticsProductProductionData")
@Api(value = "产品产量数据controller", tags = "产品产量管理")
public class ProductionManageStatisticsProductProductionDataController {
    ...
    @PostMapping("/export")
    @ApiOperation(value = "产品产量数据导出", notes = "产品产量数据导出")
    @ApiImplicitParam(name = "super-token", paramType = "header", defaultValue = "64b379cd47c843458378f479a115c322", value = "token信息", required = true)
    public void export(SearchPPDRequestDTO requestDTO, HttpServletResponse response) throws SuperCodeException {
        ...
        if (StringUtils.isNotBlank(requestDTO.getProductLevelCode())) {
            ...
            nonService.export(requestDTO, response);
        }
    }
}