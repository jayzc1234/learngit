 ...
 */
@RestController
@RequestMapping(VALID_PATH+"/repairManage")
@Api(value = "维修管理", tags = "维修管理")
public class ProductionManageRepairManageController extends CommonUtil {
    private static final String PDF_SUFFIX = "%s.pdf";
    ...
    public void exportPdf(@RequestParam("id") Long id, HttpServletResponse response) throws Exception {
        response.setHeader("Content-disposition", "attachment;filename="+ URLEncoder.encode("维修管理.pdf","utf-8"));
    ...
    public void export(ProductionManageRepairManageListDTO listDTO, HttpServletResponse response) throws Exception {
        service.exportExcelList(listDTO, getExportNumber(), "维修管理",ProductionManageRepairManageListVO.class, response);
    }
}
