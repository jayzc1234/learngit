 ...
 */
@RestController
@RequestMapping(VALID_PATH+"/statistics/stock-loss")
@Api(tags = "库存报损分析")
@Slf4j
public class StockLossStatisticsController {
    ...
    public RestResult<SearchStockLossStatisticsResponseVO> listStatistics(@RequestBody @Validated SearchStockLossPageRequestDTO requestDTO) {
    ...
    @PostMapping("/export")
    @ApiOperation(value = "库存报损列表导出", notes = "库存报损的列表导出")
    @ApiImplicitParam(name = "super-token", paramType = "header", defaultValue = "64b379cd47c843458378f479a115c322", value = "token信息", required = true)
    public void export(ExportStockLossStatisticsPageRequestDTO requestDTO, HttpServletResponse response) throws SuperCodeException {
        ...
        CustomAssert.isFalse(LocalDate.parse(requestDTO.getStartQueryDate()).isAfter(LocalDate.parse(requestDTO.getEndQueryDate())),
                "开始日期不可大于结束日期");
        service.exportStockLoss(requestDTO, response);
    }
}